...
narrowing is gated on positive inventory *and* on the surface having been measured
...
which is *not* literal pg_dump text (the CLI rewrites it: `CREATE OR REPLACE`,
...
1. **Auditability.** SIDECAR.md renders `file:line` verbatim and requires refs to be
   self-identifying. A dump is a real local file a reviewer can open; no synthetic
   `db://` URI scheme is needed.
...
3. **Build-versus-integrate, applied to the export half.** The standing directive
   ...
   artefact in one command. (The same directive applies to the *analysis* half — see the
   ...
   no dependencies through them, and a function survives a column drop *broken* — and
...
4. **A rejected alternative, named:** the Supabase management API (browser OAuth, no local
   ...
   content arguably satisfies "data stays local". It is rejected *as the contract* because
   it is platform-specific (violates reason 2) and builds an exporter (violates reason 3).
   It stays open as a possible convenience *front-end* that produces the same dump file
   (§11).
...
statements *inside* them individually, with control flow (`DECLARE`, `IF`, assignments)
...
3. **SQL-editor fallback** — deferred, as in v1, but the persona gap is now measured
   rather than suspected: *neither* path above is truly non-technical. Whether this tool
...
views, policies and generated columns do *not* survive a referenced column's drop (the
drop fails without `CASCADE`), while function bodies *do* survive, broken — the server
...
> own — a view, a function body, a policy, a trigger — whose text *reads or writes* the
...
> `undecidable`), but it *can* pull a column out of `unwired` — which is precisely
...
This is ADR 0002's declaration-is-never-proof discipline *argued*, not merely restated:
...
| trigger `UPDATE OF col` event lists       | a firing condition, not an access — fires on the column's *mention* in a SET list and reads nothing                                           | `indirect` (downgraded from v1's `exact`)      |
...
- **The view-expansion warning.** pg_dump renders views in rewritten form: a
  `SELECT *` is already expanded to an explicit column list at view-creation time, so v1's
  "`SELECT *` emits a `*` row" mitigation can never fire — no star survives into the dump.
  ...
  *view* is the move that unlocks the table, so view-derived rows carry a distinguishing
...
- **Schema qualification, stated because the sidecar row has no schema field.** The
  ...
  direction: a function, view or policy *residing* in a non-target schema may still read
...
- **Policies that reference no columns.** The live-catalog survey found most policy quals
  ...
  That is fine, not a gap: the called function's *body* carries the column evidence, and
...
transitive side, while *suppressing* evidence fails dangerous ("safe to drop") — which is
...
- **Line numbers shift on every regeneration** — any schema change (or a pg_dump version
  ...
  also names the containing object *inside* the row's `method` value — the contract
...
- **Openability is load-bearing** (it is §2 reason 1), so the recommended convention is a
  stable filename (`db-config.sql`) that is *committed* where the repo's policy allows —
  a gitignored dump makes every ref unresolvable to anyone but whoever ran it. Where it
  must stay untracked, the sidecar's snapshot identity (§7) is the fallback audit path.
...
**1. A positive inventory, required.** Caveats count what was *seen*; they are silent
...
per-class counts of objects *found* (tables, functions, views, policies, triggers), and §9's
invisible-surface narrowing is gated on inventory, not on the sidecar's mere presence.
Inventory is necessary and not sufficient: a surface that was *partially* measured is not
a measured surface. Where the producer's blindness is attributable to a table — an
unparsed statement inside a body whose binding is known, a skipped PL/pgSQL construct that
could carry column refs in expression position, an unresolved column inside a resolved
table — it is expressed as a table-scoped `*` row per rule 2, so those columns land in
`undecidable` rather than `unwired`. Where it is attributable to no table —
...
**2. Per-column loudness through rows, not integers.** A resolved table with an
unresolved column emits a table-scoped `*` row *plus* a caveat — the pattern the reference
producer already ships — never a caveat alone. A global integer cannot discharge "has to
say so, per column"; a `*` or `indirect` row can, by landing the column in `undecidable`.

**3. The caveat classes**, extended by review:

- `unparsedStatements` — statements the producer's own splitter isolated but could not parse
- `plpgsqlStatementAccounting` — per body: N embedded SQL statements parsed of M isolated;
  control flow (`DECLARE`, `IF`, assignments) is skipped *and counted*, since PL/pgSQL is
...
- `unresolvedTableNames` / `ambiguousColumnBindings` — which table, and *which table a
  resolved column belongs to* in a multi-table join; mis-binding `id` in a join is G8 by
  another route
...
- Identifier-case normalisation (sqlglot normalises; generated types do not) is a stated
  ...
  the *file*, not one object — a shifted boundary silently corrupts every `file:line`
...
is *manual inspection of the sidecar JSON* until a consumer change surfaces it (§9).
...
- **Not a schema-diff or migration tool.** The dump is never replayed and never compared
  ...
  migration.
- **No live connection in this producer's v1 contract.** Scoped rather than absolute,
  per review: a future drop-planning consumer has a legitimate claim on `pg_depend`, which
...
2. **A SQL-surface acceptance corpus.** This design proposes six detector classes with
   ...
   detectors shipped unmeasured, and whose T1 states the corpus *gates* detector work. The
...
1. **Cross-column generated expressions** — column B generated from A executes a read of
   A on every write of B. Tier 2 caps it at `indirect` for v1; arguably it belongs in
   tier 1 (`exact`). Decide with acceptance-corpus cases rather than argument.
