...
| B8  | fn with a parameter named `status` shadowing a real column of `public.jobs`; body `UPDATE public.jobs SET note='x' WHERE status='open'` | exact read `jobs.status` — the column row WINS. Resolution is CATALOG-FIRST: an unqualified identifier that matches a known column of the statement's resolved table is that column; only a non-column that matches a parameter/DECLARE name is a variable. Suppressing the column would be evidence suppression, the false-dead direction |
