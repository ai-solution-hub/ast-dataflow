...
**Status: authored as the §10.2 gate of
[`db-config-export.md`](./db-config-export.md); the implementation loop treats this file
as the test oracle source.** Every case below becomes a fixture + assertion in
`scripts/tests/test_ast_dataflow_py_db_config.py` before any detector code is written —
the T1 lesson applied to the SQL surface. Fixtures are pg_dump-style SQL snippets;
expectations are sidecar rows `(table, column, direction, confidence, method)` plus caveat
and inventory fields, in the style of the existing declarative-writes tests.
...
- The **consumer-side** behaviours: inventory-gated `invisibleSurfaces` narrowing, the
  new source key, `method` surfacing (design §9's three consumer changes). Those get their
  own TS tests in the consumer loop — and per ratification note 1, the source key and the
