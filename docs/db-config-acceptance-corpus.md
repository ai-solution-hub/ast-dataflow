...
| A9  | `LANGUAGE sql` fn with a PG14+ SQL-standard body: `BEGIN ATOMIC UPDATE public.orders SET total=0; END`      | full support: `objectsFound.functions` counts it, exact write `orders.total` — pg_dump 14+ emits this form; silence here was a confirmed review finding         |
| A10 | `CREATE UNLOGGED TABLE public.job_queue (...)` and a `CREATE FOREIGN TABLE`, plus a fn updating `job_queue` | both tables catalogued (inventory counts them, furniture rows emitted); the update emits exact rows — never `unresolvedTableNames` for a table the dump defines |
...
| B6  | `LANGUAGE sql` fn `f(p_id uuid)`, body `UPDATE public.jobs SET status='x' WHERE id = p_id`                                       | exact write `jobs.status`, exact read `jobs.id`; NO row naming `p_id`; no caveat (a bound variable is not blindness)                                                                                                       |
| B7  | plpgsql fn, `DECLARE v_status text;`, body `SELECT id FROM public.t WHERE status = v_status`                                     | exact reads `t.id` AND `t.status`; NO row for `v_status`                                                                                                                                                                   |
| B8  | fn parameter named `status` shadowing a real column of `public.jobs`; body `UPDATE public.jobs SET note='x' WHERE status='open'` | exact read `jobs.status` — CATALOG-FIRST resolution: an identifier that is a known column of the resolved table emits the column row; parameter/DECLARE suppression applies only to identifiers that are NOT known columns |
| B9  | plpgsql: `BEGIN ...; EXCEPTION WHEN OTHERS THEN INSERT INTO public.error_log(msg) VALUES (SQLERRM); END`                         | exact write `error_log.msg` — SQL after `THEN` in an EXCEPTION (or CASE) arm is a statement, never swallowed as control flow; `SQLERRM` is a variable, no fabricated row                                                   |
| B10 | sql body joining `public.orders o` and `public.users u` with unqualified parameter `p_org` in the WHERE                          | no row naming `p_org` on EITHER table — variable suppression must hold in multi-table scope; the real join-column reads still emit                                                                                         |
...
| C12 | trigger body: `INSERT INTO public.audit(order_id, amount) VALUES (NEW.id, NEW.total)` (bound to `orders`) | exact writes `audit.order_id`, `audit.amount` AND exact reads `orders.id`, `orders.total` — column reads inside VALUES tuples must not be dropped           |
| C13 | trigger from C1 plus `ALTER TABLE public.docs DISABLE TRIGGER ALL` (and a USER-form variant)              | every trigger on the table treated disabled: rows capped `indirect`, `disabledObjects` counts them — `ALL`/`USER` are keywords, never a trigger named "all" |
| C14 | BEFORE trigger, body: `NEW.updated_at = now();` (the `=` assignment spelling)                             | exact WRITE `docs.updated_at` — identical to C2's `:=`; emitting a read here inverts the direction on a very common idiom                                   |
...
| D5  | view `AS SELECT name FROM cats UNION ALL SELECT name FROM dogs`, plus a fn reading the view   | the chase resolves through the union: exact reads `cats.name` AND `dogs.name` — set-operation views must not silently break the D4 guarantee |
| D6  | a view whose stored definition retains a `*` projection (hand-authored input), chased through | table-scoped `*` read on the base table — degraded, never silent                                                                             |
...
| E5  | policy on `public.projects`: `USING (owner_id = auth.uid() OR EXISTS (SELECT 1 FROM public.members m WHERE m.user_id = auth.uid()))` | exact read `projects.owner_id` (outer unqualified columns bind to the POLICY's table, never the subquery's) AND exact read `members.user_id`; NO `members.owner_id` row |
...
| H2  | same as H1 but parentage declared via `CREATE TABLE public.orders_2024 (...)` + `ALTER TABLE ONLY public.orders ATTACH PARTITION public.orders_2024 ...` — the form pg_dump has emitted since PostgreSQL 12 | identical expectations to H1 — the ATTACH form is the one real dumps contain                                                                                                                                                                                          |
...
| I9  | a multi-line `/* block comment */` between a function's `LANGUAGE` clause and its `AS $$` body                   | every body row's `line` still matches the ORIGINAL dump line — comment newlines must survive the splitter's buffer (a confirmed review finding: the `--` branch preserved newlines, the `/* */` branch dropped them) |
| I10 | `--schema` naming a schema with no objects in the dump                                                           | hard error naming the schemas the dump DOES contain — a typo must never yield a clean empty sidecar                                                                                                                  |
| I11 | `--dump` pointing at a directory or unreadable path                                                              | clean structured error, never a traceback                                                                                                                                                                            |
