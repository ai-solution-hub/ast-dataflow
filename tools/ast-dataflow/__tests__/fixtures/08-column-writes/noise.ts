...
// None of these calls should appear in column-writes for table='survey_questions',
// column='project_id':
//   1. .from('other_table').insert({ project_id }) — wrong table, suppress.
//   2. .from('survey_questions').insert({ other_column: 'x' }) — wrong column, suppress.
//   3. Bare string literal 'project_id' — not in a Supabase write chain, suppress.
//   4. .from('survey_questions').update({ project_id }) — correct table + column,
